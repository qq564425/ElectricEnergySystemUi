...
export default {
     ...
     loadData(){
      ...
      this.$ajax.post(
        `${BaseURL}/powerData/getPowerDataList`,{
          ...
          "tableNumber":that.$store.state.moduleSearch.monthForm.tableNumber,
          "startMonth":that.$store.state.moduleSearch.monthForm.startMonth,
          "endMonth":that.$store.state.moduleSearch.monthForm.endMonth,
        }
      ).then(res => {
   ...
   computed: {
     monthSearchFlag(){
       return this.$store.state.moduleSearch.monthSearchFlag;
     }
   },

   watch: {
     monthSearchFlag(newval,oldval){
        if(newval){
           this.loadData();
           this.changeMonthSearchFlag(false);
        }
     }
   },
}
