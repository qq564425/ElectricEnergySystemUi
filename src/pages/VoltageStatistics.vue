  ...
  <div style="border-radius:5px;">
         <div style="background-color:#FFFFFF;box-shadow: 2px 2px 5px #888888;overflow: hidden;border-radius:5px;">
            <div style="background-color:#5CACEE;padding:5px;overflow:hidden;height:33px;color:white;">
               <span style="font-size:1.2em;float:left;">三相电压统计</span>
            </div>
            </br>
    ...
    <div style="box-shadow: 2px 2px 5px #888888;background-color:#FFFFFF;border-radius:5px;margin-top:20px">
        ...
        <div style="width:100%;height:40px;text-align:center;line-height:40px;font-size:18px" v-show="radio==3?true:false">
          <strong>{{startDateTitle}} 至 {{endDateTitle}} 三相电压日统计</strong>
        </div>
        <div style="width:100%;height:40px;text-align:center;line-height:40px;font-size:18px" v-show="radio==1?true:false">
          <strong>{{startMonthTitle}} 至 {{endMonthTitle}} 三相电压月统计</strong>
        </div>
        <!-- 折线图 -->
        <div id="chartmainlineHour" style="width:99%; height:600px;" v-show="radio==6?true:false"></div>
        <div id="chartmainlineDate" style="width:99%; height:600px;" v-show="radio==3?true:false"></div>
        <div id="chartmainlineMonth" style="width:99%; height:600px;" v-show="radio==1?true:false"></div>
    </div>
</div>
</template>
<script>
...
export default({
    ...
    mounted() {
       this.findAnyData('chartmainlineHour');
    },

    methods: {
      drawLine: function(type,hengxiang,aQuadrantVoltageList,bQuadrantVoltageList,cQuadrantVoltageList){
         let option = {
            tooltip : {
                trigger: 'axis'
            },
            ...
            series : [
                {
                    name:'A相电压',
                    type:'line',
                    data:aQuadrantVoltageList
                },

                {
                    name:'B相电压',
                    type:'line',
                    data:bQuadrantVoltageList
                },

                {
                    name:'C相电压',
                    type:'line',
                    data:cQuadrantVoltageList
                }
            ]
        };
    ...
    findAnyData(type){
        ...
        .then(res => {
          ...
          var aQuadrantVoltageList = res.data.aQuadrantVoltageList;
          var bQuadrantVoltageList = res.data.bQuadrantVoltageList;
          var cQuadrantVoltageList = res.data.cQuadrantVoltageList;
          that.drawLine(type,hengxiang,aQuadrantVoltageList,bQuadrantVoltageList,cQuadrantVoltageList);
        })
      ...
      submitForm(){
        ...
        this.endMonthTitle = format(this.form.endMonth,'yyyy-MM');
          if(this.radio == 3){
              ...
              this.findAnyData('chartmainlineDate');
              ...
              this.findAnyData('chartmainlineMonth');
          }
      },
