  ...
  <div style="border-radius:5px;">
         <div style="background-color:#FFFFFF;box-shadow: 2px 2px 5px #888888;overflow: hidden;border-radius:5px;">
            <div style="background-color:#5CACEE;padding:5px;overflow:hidden;height:33px;color:white;">
               <span style="font-size:1.2em;float:left;">象限电能统计</span>
            </div>
            </br>
    ...
    <div style="box-shadow: 2px 2px 5px #888888;background-color:#FFFFFF;border-radius:5px;margin-top:20px">
        ...
        <div style="width:100%;height:40px;text-align:center;line-height:40px;font-size:18px" v-show="radio==6?true:false">
          <strong>{{startDateTitle}} 至 {{endDateTitle}} 象限电能小时统计</strong>
        </div>
        <div style="width:100%;height:40px;text-align:center;line-height:40px;font-size:18px" v-show="radio==3?true:false">
          <strong>{{startDateTitle}} 至 {{endDateTitle}} 象限电能日统计</strong>
        </div>
        <div style="width:100%;height:40px;text-align:center;line-height:40px;font-size:18px" v-show="radio==1?true:false">
          <strong>{{startMonthTitle}} 至 {{endMonthTitle}} 象限电能月统计</strong>
        </div>
        <!-- 折线图 -->
        <div id="chartmainlineHour4" style="width:99%; height:600px;" v-show="radio==6?true:false"></div>
        <div id="chartmainlineDate4" style="width:99%; height:600px;" v-show="radio==3?true:false"></div>
        <div id="chartmainlineMonth4" style="width:99%; height:600px;" v-show="radio==1?true:false"></div>
    </div>
</div>
</template>
<script>
...
export default({
    ...
    mounted() {
       this.findAnyData('chartmainlineHour4');
    },

    methods: {
      drawLine: function(type,hengxiang,oneQuadrantReactivePowerList,twoQuadrantReactivePowerList,threeQuadrantReactivePowerList,fourQuadrantReactivePowerList){
         let option = {
            tooltip : {
                trigger: 'axis'
            },
            legend: {
                data:['正向有功总电能','二象限无功总电能','三象限无功总电能','四象限无功总电能']
            },
            toolbox: {
                show : true,
                feature : {
            ...
            series : [
                {
                    name:'一象限无功总电能',
                    type:'line',
                    data:oneQuadrantReactivePowerList
                },

                {
                    name:'二象限无功总电能',
                    type:'line',
                    data:twoQuadrantReactivePowerList
                },

                {
                    name:'三象限无功总电能',
                    type:'line',
                    data:threeQuadrantReactivePowerList
                },

                {
                    name:'四象限无功总电能',
                    type:'line',
                    data:fourQuadrantReactivePowerList
                }
            ]
        };
    ...
    findAnyData(type){
        ...
        .then(res => {
          ...
          var oneQuadrantReactivePowerList = res.data.oneQuadrantReactivePowerList;
          var twoQuadrantReactivePowerList = res.data.twoQuadrantReactivePowerList;
          var threeQuadrantReactivePowerList = res.data.threeQuadrantReactivePowerList;
          var fourQuadrantReactivePowerList = res.data.fourQuadrantReactivePowerList;
          that.drawLine(type,hengxiang,oneQuadrantReactivePowerList,twoQuadrantReactivePowerList,threeQuadrantReactivePowerList,fourQuadrantReactivePowerList);
        })
      ...
      submitForm(){
        ...
        this.endMonthTitle = format(this.form.endMonth,'yyyy-MM');
          if(this.radio == 3){
              ...
              this.findAnyData('chartmainlineDate4');
              ...
              this.findAnyData('chartmainlineMonth4');
          }
      },
