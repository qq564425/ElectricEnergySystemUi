  ...
  <div style="border-radius:5px;">
         <div style="background-color:#FFFFFF;box-shadow: 2px 2px 5px #888888;overflow: hidden;border-radius:5px;">
            <div style="background-color:#5CACEE;padding:5px;overflow:hidden;height:33px;color:white;">
               <span style="font-size:1.2em;float:left;">正反向电能统计</span>
            </div>
            </br>
    ...
    <div style="box-shadow: 2px 2px 5px #888888;background-color:#FFFFFF;border-radius:5px;margin-top:20px">
        ...
        <div style="width:100%;height:40px;text-align:center;line-height:40px;font-size:18px" v-show="radio==6?true:false">
          <strong>{{startDateTitle}} 至 {{endDateTitle}} 正反向电能小时统计</strong>
        </div>
        <div style="width:100%;height:40px;text-align:center;line-height:40px;font-size:18px" v-show="radio==3?true:false">
          <strong>{{startDateTitle}} 至 {{endDateTitle}} 正反向电能日统计</strong>
        </div>
        <div style="width:100%;height:40px;text-align:center;line-height:40px;font-size:18px" v-show="radio==1?true:false">
          <strong>{{startMonthTitle}} 至 {{endMonthTitle}} 正反向电能月统计</strong>
        </div>
        <!-- 折线图 -->
        <div id="chartmainlineHour3" style="width:99%; height:600px;" v-show="radio==6?true:false"></div>
        <div id="chartmainlineDate3" style="width:99%; height:600px;" v-show="radio==3?true:false"></div>
        <div id="chartmainlineMonth3" style="width:99%; height:600px;" v-show="radio==1?true:false"></div>
    </div>
</div>
</template>
<script>
...
export default({
    ...
    mounted() {
       this.findAnyData('chartmainlineHour3');
    },

    methods: {
      drawLine: function(type,hengxiang,positiveActivePowerList,reverseActivePowerList,positiveReactivePowerList,reverseReactivePowerList){
         let option = {
            tooltip : {
                trigger: 'axis'
            },
            legend: {
                data:['正向有功总电能','反向有功总电能','正向无功总电能','反向无功总电能']
            },
            toolbox: {
                show : true,
                feature : {
            ...
            series : [
                {
                    name:'正向有功总电能',
                    type:'line',
                    data:positiveActivePowerList
                },

                {
                    name:'反向有功总电能',
                    type:'line',
                    data:reverseActivePowerList
                },

                {
                    name:'正向无功总电能',
                    type:'line',
                    data:positiveReactivePowerList
                },

                {
                    name:'反向无功总电能',
                    type:'line',
                    data:reverseReactivePowerList
                }
            ]
        };
    ...
    findAnyData(type){
        ...
        .then(res => {
          ...
          var positiveActivePowerList = res.data.positiveActivePowerList;
          var reverseActivePowerList = res.data.reverseActivePowerList;
          var positiveReactivePowerList = res.data.positiveReactivePowerList;
          var reverseReactivePowerList = res.data.reverseReactivePowerList;
          that.drawLine(type,hengxiang,positiveActivePowerList,reverseActivePowerList,positiveReactivePowerList,reverseReactivePowerList);
        })
      ...
      submitForm(){
        ...
        this.endMonthTitle = format(this.form.endMonth,'yyyy-MM');
          if(this.radio == 3){
              ...
              this.findAnyData('chartmainlineDate3');
              ...
              this.findAnyData('chartmainlineMonth3');
          }
      },
