<template>
  <div style="box-shadow: 2px 2px 5px #888888;background-color:#FFFFFF;border-radius:5px;">
    <div style="background-color:#5CACEE;padding:5px;overflow:hidden;height:33px;color:white;">
      <span style="font-size:1.2em;float:left;">日耗电量</span>
    </div>
...
export default {
   ...
   methods: {
     ...mapActions({
          changeDaySearchFlag: 'changeDaySearchFlag'
      }),
     ...
     loadData(){
      ...
      this.$ajax.post(
        `${BaseURL}/powerData/getPowerDataList`,{
          ...
          "tableNumber":that.$store.state.moduleSearch.dayForm.tableNumber,
          "startDate":that.$store.state.moduleSearch.dayForm.startDate,
          "endDate":that.$store.state.moduleSearch.dayForm.endDate,
        }
      ).then(res => {
   ...
   computed: {
     daySearchFlag(){
       return this.$store.state.moduleSearch.daySearchFlag;
     }
   },

   watch: {
     daySearchFlag(newval,oldval){
        if(newval){
           this.loadData();
           this.changeDaySearchFlag(false);
        }
     }
   },
}
